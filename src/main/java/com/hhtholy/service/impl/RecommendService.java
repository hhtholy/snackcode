...
import com.hhtholy.utils.CustomSetList;
...
public class RecommendService {
    ...
    /*public List<Product> recommedProducts(User user,List<Order_> orders){
        ...
        if(user == null){ //如果用户没有登录的话  不需要推荐  返回随机的4个产品即可
               ...
               while(flag <= 3){
                   ...
                   Product product = productService.getProductIsOnLine(index);
                   if(product != null){
    ...
    public List<Product> recommedProducts(User user,List<Order_> orders){
        ...
        int min = productService.getMinId();
        Random random = new Random();
        CustomSetList<Product> products = new CustomSetList<>(); //推荐的产品 不能重复
        ...
        if(user == null){ //如果用户没有登录的话  不需要推荐  返回随机的4个产品即可
            ...
            if(products.size() < 4){  //因为随机可能重复 （list不会加进去）所以这边 进行再次判断
                for (int i = 0; i < 10; i++) { //这里再循环十次  应该基本不会十次都重复
                    if(products.size() == 4){
                        break;
                    }
                    int index = random.nextInt(max-min + 1) + min; //目前随机前20
        ...
        List<CustomSetList<String>> arrList = new ArrayList<>();//临时集合
        ...
        for (Order_ order : orders) {
            ...
            if(arrList.size() >= 1){ //添加第二列的时候 判断是否为同一个用户
                for (CustomSetList<String> list : arrList) { //CustomSetList 避免重复
                    String userId = list.get(0); //用户id
                    if(String.valueOf(id).equals(userId)){ // 如果是的话
                        /////////////////////
                        List<OrderItem> orderItems = order.getOrderItems();//获取订单对应的订单项
                        for (OrderItem orderItem : orderItems) { //遍历每一个订单项
                            ...
                            Integer isDelete = product.getIsDelete(); //上架商品
                            Integer isputAway = product.getIsputAway();
                            if(isDelete == null && isputAway.equals(1)){
                                list.add(product.getName());
                            }
                        }
                        flag1 = true;
            ...
            if(!flag1){ //如果不是同一个用户
                CustomSetList<String> list = new CustomSetList<>();//temp CustomSetList避免重复
                list.add(String.valueOf(userResult.getId())); //每个子集合 第一个添加元素为 用户的id
                List<OrderItem> orderItems = order.getOrderItems();//获取订单对应的订单项
                for (OrderItem orderItem : orderItems) { //遍历每一个订单项
                    Product product = orderItem.getProduct(); //获取产品
